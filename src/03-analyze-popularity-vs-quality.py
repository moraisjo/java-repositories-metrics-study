#!/usr/bin/env python3
"""
RQ01 pipeline: popularity (stars) vs. quality (CK metrics).
...
def load_repositories(repo_csv: Path) -> pd.DataFrame:
    required_columns = {"nameWithOwner", "stargazerCount", "age", "ckMetricsGenerated"}
    repos = pd.read_csv(
        repo_csv,
...
def build_dataset(repos: pd.DataFrame, ck_summary: pd.DataFrame) -> pd.DataFrame:
    data = (
        ...
        .assign(
            stars=lambda d: d["stargazerCount"],
            stars_log10=lambda d: np.log10(d["stargazerCount"].clip(lower=1)),
        )
    )
    if data.empty:
...
def export_report_assets(
    ...
    assets_dir = report_dir / "assets" / "rq01"
    ...
    for fig in copied_figs:
        ...
        title = fig.stem.replace("rq01_", "").replace("_scatter", "").upper()
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Analyze popularity vs CK quality metrics and export RQ01 artifacts."
    )
    parser.add_argument(
        "--project-root",
...
def main() -> None:
    ...
    corr_path = summary_dir / "rq01_correlacoes.csv"
    ...
    if not args.skip_report:
        md_path = export_report_assets(project_root, figure_paths, corr_df, corr_path, report_dir)
