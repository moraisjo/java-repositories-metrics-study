#!/usr/bin/env python3
"""
RQ02 pipeline: maturity (age in years) vs. quality (CK metrics).
...
def build_dataset(repos: pd.DataFrame, ck_summary: pd.DataFrame) -> pd.DataFrame:
    data = (
        ...
        .assign(
            maturity_years=lambda d: d["age_years"],
        )
    )
    if data.empty:
...
def export_report_assets(
    ...
    assets_dir = report_dir / "assets" / "rq02"
    ...
    corr_dst = assets_dir / "rq02_correlacoes.csv"
    if corr_path.exists():
        shutil.copy2(corr_path, corr_dst)

    ck_summary_dst = assets_dir / "rq02_resumo_por_repositorio.csv"
    ...
    md_path = report_dir / "rq02_figuras.md"
    lines = [
        "# RQ02 - Graficos e Tabela de Correlacao",
        "",
        "## Correlacoes",
        "",
        corr_table_md,
    ...
    for fig in copied_figs:
        ...
        title = fig.stem.replace("rq02_", "").replace("_scatter", "").upper()
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Analyze maturity vs CK quality metrics and export RQ02 artifacts."
    )
    parser.add_argument(
        "--project-root",
...
def main() -> None:
    ...
    corr_path = summary_dir / "rq02_correlacoes.csv"
